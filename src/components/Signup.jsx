...
const Signup = ({onClick , isLogin , onSignup}) => {
    ...
    const signupHandler = (e) => {
        ...
        if (email === '' || password === '' || name === '' || conformPassword === '') {
            ...
            Axios.post('/users/signup', { email, password, name }).then((res) => {
  ...
  return (
    ...
    <div className={`-translate-y-100 opacity-0  absolute p-4 w-11/12 md:w-1/2 lg:w-1/4 mx-4 bg-white rounded-lg border border-gray-200 shadow-md sm:p-6 lg:p-8 dark:bg-gray-800 dark:border-gray-700 transition-all duration-1000 ${isLogin && 'translate-y-0 opacity-100'}`}>
        ...
        <form className="space-y-6" onSubmit={signupHandler}>
            <h5 className="text-xl font-anak font-bold uppercase dark:text-white text-center text-blue-800 ">Create account</h5>
            <div>
                <label for="email" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300 capitalize">email</label>
            ...
            <div>
                <label for="name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300 capitalize">name</label>
                <input onChange={(e) => setname(e.target.value)} value={name} type="text" name="name" id="usernaem" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="your name here..." required="" />
            </div>
            <div>
            ...
            <div>
                <label for="conform-password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300 capitalize">conform password</label>
