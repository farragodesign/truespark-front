...
const RelatedContents = ({ categoryId, articleId }) => {
  ...
  useEffect(() => {
    Axios.get(
      `/articles/find-exeption/${categoryId && categoryId._id}/${
        articleId && articleId
      }`
    )
      .then((res) => {
  ...
  return (
    <div className="flex flex-col">
      <div>
        <h2 className="text-xl mt-4 font-bold text-center uppercase">
          Related Contents
        </h2>
      </div>

      {relatedContents ? (
        relatedContents.map((data, i) => {
          return (
            <div class="flex mt-4 h-36 w-full overflow-hidden items-center bg-white rounded-lg border shadow-md md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
              <Link
                to={`/article/${data._id}`}
                className="w-1/2 h-full overflow-hidden rounded-t-lg md:rounded-none md:rounded-l-lg"
              >
